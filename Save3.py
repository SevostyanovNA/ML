...
def read_vrp_file(file_path):
    """Чтение файла .vrп и извлечение данных в зависимости от формата"""
...
def validate_routes(routes, num_nodes):
    """Проверка корректности маршрутов и обработки всех точек"""
    visited_nodes = set()
    repeated_nodes = set()
    for route in routes:
        for node in route:
            if node == 1:  # Пропускаем депо
                continue
            if node in visited_nodes:
                repeated_nodes.add(node)
            visited_nodes.add(node)
    if repeated_nodes:
        raise ValueError(f"Некорректные маршруты: точки повторяются {repeated_nodes}. Маршруты: {routes}")
    if len(visited_nodes) != num_nodes - 1:  # Все точки, кроме депо
        missing_nodes = set(range(2, num_nodes + 1)) - visited_nodes
        raise ValueError(f"Некорректные маршруты: не все точки были посещены. Пропущенные точки: {missing_nodes}. Маршруты: {routes}")
...
def solve_cvrp_with_aco(data, dist_matrix, num_ants=20, alpha=1.0, beta=5.0, evaporation_rate=0.7, num_iterations=500, stop_criteria=10):
    ...
    # Функция 2-opt для локального поиска
    def two_opt(route, dist_matrix):
        best_route = route[:]
        best_cost = calculate_routes_cost([route], dist_matrix)
        improved = True
        while improved:
            improved = False
    ...
    # Жадный алгоритм для инициализации феромонов
            ...
            while True:
                ...
                for next_node in range(num_nodes):
                    if next_node not in visited and next_node != depot and current_load + demands[next_node] <= capacity:
                        cost = dist_matrix[current_route[-1]][next_node]
                        if cost < best_cost:
                            best_node = next_node
                            best_cost = cost
    ...
    # Инициализация феромонов на основе жадного решения
    pheromones = np.ones((num_nodes, num_nodes))
    initial_routes = greedy_initial_solution()
    initial_cost = calculate_routes_cost(initial_routes, dist_matrix)
    for route in initial_routes:
        for i in range(len(route) - 1):
            pheromones[route[i]][route[i + 1]] += 1.0 / (initial_cost + 1e-6)
...
def load_or_create_params(file_name, data, dist_matrix, param_dir):
    ...
    for key, (data_dir, param_dir) in directories.items():
        for file_name in os.listdir(data_dir):
            if file_name.endswith('.vrp'):
                ...
                params = load_or_create_params(file_name, data, dist_matrix, param_dir)
                best_params = params['best_params']
                best_cost = params['best_cost']
